...
const CreateCard: React.FC<Props> = (props) => {
  ...
  const toggleImageRequest = useCallback((value: boolean) => {
    setRequestImage(value);
  }, []);
  ...
  return (
    <>
      {requestImage && (
        <ImagePicker
          onImagePickCancelled={() => toggleImageRequest(false)}
          onImagePicked={() => toggleImageRequest(false)}
        />
      )}
      <Card
        body={
          <View style={styles.container}>
            <TouchableOpacity
              ...
              onPress={() => toggleImageRequest(true)}
            >
